...
// [FIXED: frame < p + 12 condition was eaten; return JSX was stripped]
// -----------------------------------------------------------------------------
const AmbientPulse = () => {
  const frame = useCurrentFrame();
  ...
  const pulseOpacity = interpolate(frame - nearestPulse, [0, 4, 12], [0, 0.06, 0], { extrapolateRight: 'clamp' });
  if (pulseOpacity <= 0) return null;
  return (
    <AbsoluteFill style={{
...
// [FIXED: fadeOpacity <= 0 condition was eaten; return JSX was stripped]
// -----------------------------------------------------------------------------
const OutroFade = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{ background: '#000', opacity: fadeOpacity, pointerEvents: 'none' }} />
  );
};
...
const EndFade = () => {
  const frame = useCurrentFrame();
  const opacity = interpolate(frame, [870, 900], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });

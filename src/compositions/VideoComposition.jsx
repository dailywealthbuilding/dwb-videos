// src/compositions/VideoComposition.jsx — DWB Composition v3.1 FIXED
// Fixes applied:
//   - Removed duplicate LowerThird import (was defined locally AND imported)
//   - Added missing Easing import
//   - Reconstructed all eaten JSX returns (GlowBorder, ProgressBar, CornerBrackets, etc.)
//   - Fixed CornerBrackets/ProgressBar prop mismatch (accentColor -> videoId)
//   - Fixed MatrixRain opacity (uses day-specific opacity from special layer)
...
// NOTE: LowerThird, SceneNumber, CornerTimestamp removed from SpecialOverlays import
// LowerThird is defined locally below; SceneNumber/CornerTimestamp not used in render
...
// 3. Download via yt1s.com as MP3 — rename day43.mp3 through day49.mp3
...
const COLOR_GRADES = {
  ...
  day39: { top: 'rgba(60,0,60,0.22)',   mid: 'rgba(30,0,30,0.06)',   bot: 'rgba(60,0,60,0.22)',   accent: '#FF44FF' },
  day40: { top: 'rgba(80,25,0,0.22)',   mid: 'rgba(40,12,0,0.06)',   bot: 'rgba(80,25,0,0.22)',   accent: '#FF6600' },
  day41: { top: 'rgba(0,30,70,0.22)',   mid: 'rgba(0,15,35,0.06)',   bot: 'rgba(0,30,70,0.22)',   accent: '#00AAFF' },
  day42: { top: 'rgba(60,50,0,0.22)',   mid: 'rgba(30,25,0,0.06)',   bot: 'rgba(60,50,0,0.22)',   accent: '#FFD700' },
  // Week 7
...
const GlowBorder = ({ videoId }) => {
  ...
  return (
    <AbsoluteFill style={{
      ...
      boxShadow: '0 0 18px ' + borderColor + ', inset 0 0 18px ' + borderColor,
      opacity,
      pointerEvents: 'none',
    }} />
  );
...
const LetterboxBars = () => {
  ...
  return (
    <AbsoluteFill style={{ pointerEvents: 'none' }}>
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: barH + 'px', background: '#000' }} />
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: barH + 'px', background: '#000' }} />
    </AbsoluteFill>
  );
};
  ...
  return (
    <AbsoluteFill style={{ pointerEvents: 'none' }}>
      <div style={{
        ...
        boxShadow: '0 0 20px ' + grade.accent.replace(')', ',' + glowPulse + ')').replace('rgb', 'rgba'),
        padding: '18px 24px',
        borderRadius: '8px',
        textAlign: 'center',
      }}>
        <div style={{ fontFamily: "'Montserrat', sans-serif", fontSize: '20px', color: 'rgba(255,255,255,0.7)', letterSpacing: '0.05em', marginBottom: '4px' }}>Follow For Daily Videos</div>
        <div style={{ fontFamily: "'Anton', sans-serif", fontSize: '30px', color: grade.accent, letterSpacing: '0.06em' }}>@DailyWealthBuilding</div>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '13px', color: 'rgba(255,255,255,0.5)', letterSpacing: '0.1em', marginTop: '6px' }}>90-Day Public Challenge · Day {dayNum} of 90</div>
      </div>
    </AbsoluteFill>
  );
};
...
const ChromaticAberration = ({ intensity = 0.6 }) => {
  ...
  return (
    <AbsoluteFill style={{ pointerEvents: 'none', mixBlendMode: 'screen' }}>
      <AbsoluteFill style={{ transform: 'translateX(-' + offset + 'px)', background: 'rgba(255,0,0,0.04)' }} />
      <AbsoluteFill style={{ transform: 'translateX(' + offset + 'px)', background: 'rgba(0,0,255,0.04)' }} />
    </AbsoluteFill>
  );
};
...
const DepthOfField = ({ overlays = [] }) => {
  ...
  for (const o of overlays) {
    // Blur in just before overlay appears, clear when overlay is showing
    if (frame >= o.startFrame - DUCK_FRAMES && frame < o.startFrame) {
      nearestBlur = interpolate(frame, [o.startFrame - DUCK_FRAMES, o.startFrame], [1.5, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
      break;
    }
    if (frame >= o.endFrame && frame < o.endFrame + DUCK_FRAMES) {
      nearestBlur = interpolate(frame, [o.endFrame, o.endFrame + DUCK_FRAMES], [0, 1.5], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
      break;
    }
  }
...
const TVStaticBurst = ({ clipBoundaryFrames = [] }) => {
  ...
  return (
    <AbsoluteFill style={{
      background: 'repeating-linear-gradient(0deg, transparent, transparent 3px, rgba(255,255,255,0.07) 3px, rgba(255,255,255,0.07) 4px)',
      opacity: 0.9,
      pointerEvents: 'none',
    }} />
  );
...
const MatrixRain = ({ opacity = 0.15 }) => {
  ...
  for (let i = 0; i < COLS; i++) {
    ...
    const charIndex = Math.floor((frame * speed + offset) % CHARS.length);
    const char   = CHARS[charIndex] || '0';
    const trail1 = CHARS[(charIndex + 1) % CHARS.length] || '1';
    const trail2 = CHARS[(charIndex + 2) % CHARS.length] || '0';
    const yPos   = ((frame * speed + offset) % 110) - 10;
    columns.push(
      <div key={i} style={{
        position: 'absolute',
        left: (i / COLS * 100) + '%',
        ...
        <div style={{ color: '#00FF88', opacity: opacity }}>{char}</div>
        <div style={{ color: '#00CC66', opacity: opacity * 0.7 }}>{trail1}</div>
        <div style={{ color: '#009944', opacity: opacity * 0.4 }}>{trail2}</div>
      </div>
    );
  }
  return (
...
export const VideoComposition = ({ videoId, overlays: propOverlays, music: propMusic }) => {
  ...
  return (
    <AbsoluteFill style={{ background: '#000' }}>
      ...
      {/* Layer 7: Letterbox bars (first 35 frames only) */}
      <LetterboxBars />

      {/* Layer 8: Shockwave ring on frame 0 */}
      <ShockwaveRing />
      ...
      {/* Layer 11: Chromatic aberration */}
      <ChromaticAberration intensity={0.6} />

      {/* Layer 12: TV static at clip boundaries */}
      <TVStaticBurst clipBoundaryFrames={CLIP_BOUNDARY_FRAMES} />

      {/* Layer 13: Matrix rain — techy days */}
      {special.matrixRain && (
        <MatrixRain opacity={special.matrixOpacity || 0.15} />
      )}

      {/* Layer 14: Depth of field blur */}
      <DepthOfField overlays={overlays} />

      {/* Layer 15: Particle system */}
      {DAY_PARTICLES[videoId] && (
        <Particles type={DAY_PARTICLES[videoId]} opacity={1} active={true} />
      )}

      {/* Layer 16: Audio */}
      <AudioTrack videoId={videoId} music={music} overlays={overlays} />

      {/* Layer 17: Corner brackets */}
      <CornerBrackets videoId={videoId} />

      {/* Layer 18: Day badge */}
      <DayBadge videoId={videoId} />

      {/* Layer 19: Text overlays */}
      ...
      {/* Layer 21: Lower third */}
      <LowerThird />

      {/* Layer 22: Progress bar */}
